...
static int parseGroup(const char *&user, char *group[], int &group_size, const char *ugi)
{
...
static int parseHostAndPort(const char *&host, unsigned short &port, const char *fs_str)
{
#define HOST_PORT_BUFF_SIZE 4096
  ...
  char *str = end;
  while (str > host_port_buff) {
    if (*str == ':' && str != end) {                          /* fs format host:port */
      ret = 0;
      break;
    }
    str--;
