...
{ 
  namespace olapdrive 
  {
    ...
    ObSeveralCreativeEffect::ObSeveralCreativeEffect(
    ...
    const char* ObSeveralCreativeEffect::get_test_name()
    {
      return "SEVERAL_CREATIVEE_FFECT";
    }

    int ObSeveralCreativeEffect::check(
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          FOREACH_CELL(scanner)
          {
            ...
            if (is_row_changed)
            {
              creative_id = scan_info.batch_scan_info_.creative_id_array_[row_index - 1];
              sum_val = creative_id * scan_info.day_count_;
            }
            if (OB_SUCCESS == ret)
            {
    ...
    int ObSeveralCreativeEffect::add_special_column(ObScanParam& scan_param)
    {
      int ret                       = OB_SUCCESS;
      ...
      if (OB_SUCCESS == ret)
      {
        ret = get_random_bidword_id_array(
          campaign, extra_info->batch_scan_info_.adgroup_id_array_, 
          extra_info->batch_scan_info_.adgroup_cnt_,
          extra_info->batch_scan_info_.creative_id_array_,
          extra_info->batch_scan_info_.creative_cnt_);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ret = groupby_param.add_groupby_column(creative_id);
      }
