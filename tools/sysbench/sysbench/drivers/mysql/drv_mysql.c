...
static sb_arg_t mysql_drv_args[] =
{
  ...
  {"mysql-port", "MySQL server port", SB_ARG_TYPE_INT, "2828"},
  {"mysql-socket", "MySQL socket", SB_ARG_TYPE_STRING, NULL},
  {"mysql-user", "MySQL user", SB_ARG_TYPE_STRING, "admin"},
  {"mysql-password", "MySQL password", SB_ARG_TYPE_STRING, "admin"},
  ...
  {"ob", "test oceanbase using mysqlclient", SB_ARG_TYPE_FLAG, "off"},
...
int mysql_drv_init(void)
{
  ...
  args.ob = sb_get_value_flag("ob"); // junyue
...
int mysql_drv_describe(drv_caps_t *caps, const char * table_name)
{
  ...
  *caps = mysql_drv_caps;
  ...
  if(args.ob)
  {
      caps->auto_increment = 0; // ob does not support auto_inc
      caps->unsigned_int = 0;   // ob does not support unsigned int
  }
  /* junyue end */
  ...
  rc = mysql_real_query((MYSQL*)con.ptr, query, strlen(query)); 
  DEBUG("mysql_real_query(%p, \"%s\", %d) = %d", con.ptr, query, strlen(query), rc);
  if (rc)
    goto error;
  
  res = mysql_store_result((MYSQL*)con.ptr);
 ...
 error:
  ...
  log_text(LOG_ALERT, "MySQL error: %s", mysql_error((MYSQL*)con.ptr));
  rc = 1;
...
int mysql_drv_bind_param(db_stmt_t *stmt, db_bind_t *params, unsigned int len)
{
  ...
  if (!stmt->emulated)
  {
    ...
    rc = mysql_stmt_param_count((MYSQL_STMT*)stmt->ptr);
    ...
    for (i = 0; i < len; i++)
    {
      bind[i].buffer_type = (enum_field_types)get_mysql_bind_type(params[i].type);
    ...
    rc = mysql_stmt_bind_param((MYSQL_STMT*)stmt->ptr, bind);
...
int mysql_drv_bind_result(db_stmt_t *stmt, db_bind_t *params, unsigned int len)
{
  ...
  rc = mysql_stmt_bind_result((MYSQL_STMT*)stmt->ptr, bind);
...
int mysql_drv_execute(db_stmt_t *stmt, db_result_set_t *rs)
{
  ...
  if (!stmt->emulated)
  {
    ...
    rc = mysql_stmt_execute((MYSQL_STMT*)stmt->ptr);
    DEBUG("mysql_stmt_execute(%p) = %u", stmt->ptr, rc);
    if (rc)
    {
      rc = mysql_errno((MYSQL*)con->ptr);
      ...
      log_text(LOG_ALERT, "failed to execute mysql_stmt_execute(): Err%d %s",
               mysql_errno((MYSQL*)con->ptr),
               mysql_error((MYSQL*)con->ptr));
      int errorcode = mysql_errno((MYSQL*)con->ptr);
      if(errorcode == 119)
          return SB_DB_ERROR_FREEZE;
      if(errorcode == 12)
          return SB_DB_ERROR_TIMEOUT;
      return SB_DB_ERROR_FAILED;
    }
  ...
  con->db_errno = (db_error_t)mysql_drv_query(con, buf, rs);
  free(buf);
...
int mysql_drv_fetch_row(db_result_set_t *rs, db_row_t *db_row)
{
  MYSQL_RES    *res = (MYSQL_RES*)rs->ptr;
  MYSQL_ROW    row;

  unsigned int num_fields = mysql_num_fields(res);
  db_row->field_count = num_fields;
  row = mysql_fetch_row(res);
  
  if(row) {
      for(int i =0 ; i < num_fields; i++) {
          db_row->values[i] =  row[i];
      }
      rs->row = db_row;
  }
  else return 1;
...
{
  MYSQL        *con = (MYSQL*)rs->connection->ptr;
  MYSQL_RES    *res;
  MYSQL_ROW    row;
  int rc;
  ...
  if (rs->statement != NULL && rs->statement->emulated == 0)
  {
    ...
    rc = mysql_stmt_store_result((MYSQL_STMT*)rs->statement->ptr);
    ...
    rs->nrows = mysql_stmt_num_rows((MYSQL_STMT*)rs->statement->ptr);
    DEBUG("mysql_stmt_num_rows(%p) = %d", rs->statement->ptr, rs->nrows);
    do {
      rc = mysql_stmt_fetch((MYSQL_STMT*)rs->statement->ptr);
  ...
  //DEBUG("mysql_num_fields(%p) = %u", res, mysql_num_fields(res));
  ...
  while((row = mysql_fetch_row(res))) {
    ...
    for(int i = 0 ; i < num_fields; i++)
    {
       //DEBUG("%.*s ", (int)lengths[i], row[i]?row[i]:"NULL");
       DEBUG("%s ",  row[i]?row[i]:"NULL");
    }
  }
  */
  return SB_DB_ERROR_NONE;
...
int mysql_drv_free_results(db_result_set_t *rs)
{
  ...
  if (rs->statement != NULL && rs->statement->emulated == 0)
  {
    ...
    return mysql_stmt_free_result((MYSQL_STMT*)rs->statement->ptr);
  }
#endif
...
int mysql_drv_close(db_stmt_t *stmt)
{
  ...
  return mysql_stmt_close((MYSQL_STMT*)stmt->ptr);
#else
  (void)stmt; /* unused */
  return 0;
#endif
...
int parse_table_engine(const char *type)
{
  ...
  if(args.ob) 
  {
    log_text(LOG_NOTICE, "OB does not support engine plugins");
  }
  /* junyue end */
  else if (!strcasecmp(type, "myisam"))
  {
