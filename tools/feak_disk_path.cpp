...
namespace oceanbase 
{
  namespace sstable
  {
    ...
    uint64_t get_ups_sstable_disk_no(const uint64_t sstable_file_id)
    {
      return (sstable_file_id & DISK_NO_MASK);
    }
    ...
    int get_sstable_directory(const int32_t disk_no, char *path, const int64_t path_len)
    {
      ...
      {
        // read from configure file
        int bufsiz = snprintf(path, path_len, "%s", g_sstable_directory);
        if (bufsiz + 1 > path_len)
        {
          TBSYS_LOG(WARN, "get_sstable_directory, path_len=%ld <= bufsiz=%d", path_len, bufsiz);
          ret = OB_SIZE_OVERFLOW;
    ...
    int get_sstable_path(const ObSSTableId& sstable_id, char *path, const int64_t path_len)
    {
      ...
      {
        // read from configure file
        int bufsiz = snprintf(path, path_len, "%s/%ld",
            g_sstable_directory, sstable_id.sstable_file_id_);
        if (bufsiz + 1 > path_len)
        {
    ...
    int get_meta_path(const int32_t disk_no, const bool current, char *path, const int32_t path_len)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (current) 
        {
          bufsiz = snprintf(path, path_len, "%s/idx_%d", 
              g_sstable_directory, disk_no);
        }
        else
        {
          time_t t;
          ...
          bufsiz = snprintf(path, path_len, "%s/idx_%d.%04d%02d%02d%02d%02d%02d", 
              g_sstable_directory, disk_no, 
    ...
    int get_tmp_meta_path(const int32_t disk_no, char *path, const int32_t path_len)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          int bufsiz = snprintf(path, path_len, "%s/idx_%d_%ld",
              g_sstable_directory, disk_no, current_time);
          if (bufsiz + 1 > path_len)
          {
        ...
        const bool current, char *path, const int32_t path_len)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (current) 
          {
            bufsiz = snprintf(path, path_len, "%s/idx_%ld_%d", 
                g_sstable_directory, version, disk_no);
          }
          else
          {
            time_t t;
            ::time(&t);
            struct tm* tm = ::localtime(&t);
            bufsiz = snprintf(path, path_len, "%s/idx_%ld_%d.%04d%02d%02d%02d%02d%02d", 
                g_sstable_directory, version, disk_no, 
                tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday,
                tm->tm_hour, tm->tm_min, tm->tm_sec);
          }
