...
#include <vector>
#include "ob_import_v2.h" //add by pangtz:20141218 //modify by zhuxh:20150722
...
//add by zhuxh:20150825 :b
//add by zhuxh:20160603 [Sequence] :b
const int ColumnInfo::DEFAULT_TOKEN_COLUMN = -1;
const int ColumnInfo::SEQUENCE = -2;
const int ColumnMapInfo::UNDEFINED = -1;
const int ColumnMapInfo::SEQUENCE = -2;
const int SubstrInfo::UNDEFINED = -1;
const int64_t SequenceInfo::UNDEFINED = LONG_LONG_MIN; //add by zhuxh:20160907 [cb Sequence]
//add :e
//add :e
...
int ImportParam::load(const char *file)
{
  ...
  if (NULL == file)
  {
    error_arr[CONF_FILE_ERROR] = false; //add by zhuxh:20150722
  ...
  if (ret == OB_SUCCESS) {
    ...
    for(size_t i = 0;i < all_sections.size() && ret == OB_SUCCESS; i++) {
      ...
      //param.input_column_nr = atoi(str_input_column_nr); //delete by zhuxh:20150727
	  ...
	  if(str_substr != NULL) {
		  ...
		  for(size_t index = 0; index < substr_info.size(); index++){
				...
				if(substr_info_original.size() != 2){
					ret = OB_ERROR;
                    error_arr[CONF_FILE_ERROR] = false; //modify by zhuxh:20150722
			  ...
			  if(one_substr_info.size() != 3){
                  ...
                  //TODO
                  //Of course the function load() should be deleted
...
int ImportParam::get_table_param(const char *table_name, TableParam &param)
{
  ...
  error_arr[GET_TABLE_PARAM_ERROR] = false; //add by zhuxh:20150722
  return OB_ERROR;
}
...
void ImportParam::PrintDebug()
{
  for(size_t i = 0;i < params_.size(); i++) {
    ...
    //fprintf(stderr, "input column nr=[%d]\n", param.input_column_nr); ////delete by zhuxh:20150727
