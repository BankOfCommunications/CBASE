...
ObImportLogInfo::ObImportLogInfo()
{
	...
	bad_lineno_ = 0;
	succ_lineno_ = 0;
  wait_time_sec_ = 0;// add by zhangcd 20150805
}
...
void ObImportLogInfo::print_error_log()
{
    ...
    fprintf(stdout, "# wait for merge done time: %lds\n", get_wait_time_sec()); // add by zhangcd 20150804
...
int ObRowBuilder::make_obobj(const ColumnInfo &column_info, ObObj &obj, TokenInfo *tokens) const
{
  ...
  //mod:e
    ...
    switch(type) {
     case ObIntType:
     case ObInt32Type: //add by zhuxh:20160907 [int32]
       {
	 //mod by liyongfeng
         int64_t lv = 0;
         if (token_len != 0)
	    ...
	    if(ret == OB_SUCCESS)
	    {
          ...
          {
            obj.set_int32( static_cast<int32_t>(lv));
          }
          //add by zhuxh:20160907 [int32] :e
	    }
	    else
	    {
		TBSYS_LOG(ERROR, "failed to transform_str_to_int, actual value[%.*s], column offset[%d]", token_len, token, token_idx);
