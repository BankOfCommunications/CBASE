...
void run_query(struct st_connection *cn, struct st_command *command, int flags)
{
  ...
  */
  char filter[8][100] = {"select","update","delete","insert","replace","begin","commit","rollback"};
  int i = 0;
  my_bool temp = match_re(&ps_re, query);
  //printf("ps_protocol_enabled => %d, complete_query => %d , match_re => %d \n", ps_protocol_enabled, complete_query, temp);
  if (ps_protocol_enabled &&
      complete_query &&
      temp) {
    //printf("use prepare api query=> %s\n", query);
    for(i = 0 ; i < 8; i++) {

        int f_len = strlen(filter[i]);
        int len = (f_len > query_len)?query_len:f_len;

        if(strncasecmp(filter[i],query,len) == 0) {
            run_query_stmt(mysql, command, query, query_len, ds, &ds_warnings);
            break;
        }
    }
    if(i == 8) {
        //printf("use normal api query=> %s\n", query);
...
int main(int argc, char **argv)
{
  ...
  //printf("===================== ps_protocol_enabled => %d ps_protocol =>  %d \n", ps_protocol_enabled, ps_protocol);
