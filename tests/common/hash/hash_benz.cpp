...
void *ht_rd_thread_func(void *data)
{
  ...
  {
    value_t value;
    if (HASH_EXIST != ght->get(thread_data->values[i].key, value))
    {
...
void *ht_wr_thread_func(void *data)
{
  ...
  for (uint64_t i = start_pos; i < end_pos; i++)
  {
    if (HASH_OVERWRITE_SUCC != ght->set((thread_data->values)[i].key, (thread_data->values)[i], 1))
    {
...
void benz_ht_set(value_t *values)
{
  ...
  for (uint64_t i = 0; i < ITEM_NUM; i++)
  {
    if (HASH_INSERT_SUCC != ght->set(values[i].key, values[i], 0))
    {
      fprintf(stderr, "set fail i=%lu\n", i);
      exit(-1);
    }
...
void benz_ht_get(value_t *values)
{
  ...
  {
    value_t value;
    if (HASH_EXIST != ght->get(values[i].key, value))
    {
      fprintf(stderr, "get fail i=%lu\n", i);
      exit(-1);
    }
