...
#define SETUP_ENV()   TBSYS_LOG(INFO, "( 1 )");    \
  ...
  async.test_init((char*)filename, (char**)scanner_section_names);    \
...
TEST(ObMergerScanEvent, parallel_count_limit_with_scanner_fulfill)
{
  const char * filename = "test_data/schema_scan_test_1.txt";

  // 2_a
  const char * scanner_section_names[] = {
    "schema_scanner_1_2",
    "schema_scanner_3_4",
    "schema_scanner_5_8"
  };
  ...
  SETUP_ENV();
  // max_parellel_count:1
  scan_event.set_request_param(param, 100000, 1);
  int i = 0;
  int ret = OB_SUCCESS;
  bool finish = false;
  ...
  TBSYS_LOG(DEBUG, "rpc_events_list_.size()=%ld", async.rpc_events_list_.get_array_index());
  for (i = 0; i < async.rpc_events_list_.get_array_index(); i++)
  {
    EXPECT_TRUE(scan_event.get_total_sub_request_count() == i+1);
    ...
    EXPECT_TRUE(scan_event.get_total_sub_request_count() == MIN(i+2,async.rpc_events_list_.get_array_index()));
    EXPECT_TRUE(scan_event.get_finished_sub_request_count() == MIN(i+1,async.rpc_events_list_.get_array_index()));
  ...
  TBSYS_LOG(INFO, "totally %ld results saved", async.rpc_events_list_.get_array_index());

}
TEST(ObMergerScanEvent, parallel_count_limit_2_with_scanner_fulfill)
{
  const char * filename = "test_data/schema_scan_test_1.txt";
  ...
  SETUP_ENV();
  scan_event.set_request_param(param, 100000, 2);
  int i = 0;
  int ret = OB_SUCCESS;
  bool finish = false;
  ...
  for (i = 0; i < async.rpc_events_list_.get_array_index(); i++)
  {
    EXPECT_TRUE(scan_event.get_total_sub_request_count() == MIN((i+1) * 2, async.rpc_events_list_.get_array_index()));
    EXPECT_TRUE(scan_event.get_finished_sub_request_count() == i);
...
TEST(ObMergerScanEvent, parallel_count_limit_2_with_scanner_not_fulfill)
{
  const char * filename = "test_data/schema_scan_test_3.txt";
...
{
  const char * filename = "test_data/schema_scan_test_4.txt";
