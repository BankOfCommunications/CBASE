...
SelectWhereCompOrderLimit SelectWhereCompOrderLimit::static_case_;
static CharArena allocator_;
namespace 
{
  static const char * comp_1st_expr = "d = 255";
  static const char * comp_1st_cname = "comp1st";
  static const char * comp_2nd_expr= "d > 254";
  static const char * comp_2nd_cname = "comp2nd";
  static const char * where_cond = "comp1st or comp2nd";
...
bool SelectWhereCompOrderLimit::check_result(const uint32_t min_key_include, const uint32_t max_key_include, ObScanner &result,
  void *arg)
{
  UNUSED(arg);
  bool res = true;
    ...
    row_key --)
  {
    ...
    ObRowkey row_key_str = make_rowkey(key, &allocator_);
...
int SelectWhereCompOrderLimit::form_scan_param(ObScanParam &param, const uint32_t min_key_include, const uint32_t max_key_include,
  void *&arg)
{
  arg = NULL;
  int err = OB_SUCCESS;
  ...
  ObString expr;
  expr.assign((char*)comp_1st_expr, static_cast<int32_t>(strlen(comp_1st_expr)));
  column_name.assign((char*)comp_1st_cname, static_cast<int32_t>(strlen(comp_1st_cname)));
  if ((OB_SUCCESS == err) && (OB_SUCCESS != param.add_column(expr,column_name,false)))
  {
    TBSYS_LOG(WARN,"fail to add composite column");
  }

  expr.assign((char*)comp_2nd_expr, static_cast<int32_t>(strlen(comp_2nd_expr)));
  column_name.assign((char*)comp_2nd_cname, static_cast<int32_t>(strlen(comp_2nd_cname)));
  if ((OB_SUCCESS == err) && (OB_SUCCESS != param.add_column(expr,column_name,false)))
  {
    TBSYS_LOG(WARN,"fail to add composite column");
  }
