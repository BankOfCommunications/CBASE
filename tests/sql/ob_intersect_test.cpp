#include "sql/ob_merge_intersect.h"
...
TEST(ObIntersectAll, test4)
{
  ...
  ObMergeIntersect intersect_all;
  intersect_all.set_distinct(false);
  ASSERT_EQ(OB_SUCCESS, intersect_all.set_child(0, first_query_result));
  ASSERT_EQ(OB_SUCCESS, intersect_all.set_child(1, second_query_result));
  ASSERT_EQ(OB_SUCCESS, intersect_all.open());
  const ObRow *row = NULL;

  ASSERT_EQ(OB_ITER_END, intersect_all.get_next_row(row));
  ASSERT_EQ(OB_ITER_END, intersect_all.get_next_row(row));
  ASSERT_EQ(OB_ITER_END, intersect_all.get_next_row(row));

}
TEST(ObIntersectAll, test3)
{
  const uint64_t left_table_id = 1001;
  const uint64_t right_table_id = 2001;
...
TEST(ObIntersectAll, test2)
{
  ...
  int64_t int_value = -1;
  //1,1,3
  ASSERT_EQ(OB_SUCCESS, intersect_all.get_next_row(row));
...
TEST(ObIntersectDistinct, test3)
{
  const uint64_t left_table_id = 1001;
  const uint64_t right_table_id = 2001;
  // empty
  ...
  ObMergeIntersect intersect_distinct;
  intersect_distinct.set_distinct(true);
  ASSERT_EQ(OB_SUCCESS, intersect_distinct.set_child(0, first_query_result));
  ASSERT_EQ(OB_SUCCESS, intersect_distinct.set_child(1, second_query_result));
  ASSERT_EQ(OB_SUCCESS, intersect_distinct.open());
  const ObRow *row = NULL;

  ASSERT_EQ(OB_ITER_END,intersect_distinct.get_next_row(row));
  ASSERT_EQ(OB_ITER_END,intersect_distinct.get_next_row(row));
  ASSERT_EQ(OB_ITER_END,intersect_distinct.get_next_row(row));
}
TEST(ObIntersectDistinct, test2)
{
  const uint64_t left_table_id = 1001;
  const uint64_t right_table_id = 2001;
  // 1
...
TEST(ObIntersectDistinct, test1)
{
  const uint64_t left_table_id = 1001;
  const uint64_t right_table_id = 2001;
  // 1,1,2,2,3,7
  ...
  // 1,3
  ASSERT_EQ(OB_SUCCESS, intersect_distinct.get_next_row(row));
  ASSERT_EQ(OB_SUCCESS, row->raw_get_cell(0, cell, table_id, column_id));
  ASSERT_EQ(OB_SUCCESS, cell->get_int(int_value));
  ASSERT_EQ(1, int_value);
  ASSERT_EQ(left_table_id, table_id);
