...
TEST(ObUnionAll, test4)
{
  ...
  ObMergeUnion union_all;
  union_all.set_distinct(false);
  ASSERT_EQ(OB_SUCCESS, union_all.set_child(0, first_query_result));
  ASSERT_EQ(OB_SUCCESS, union_all.set_child(1, second_query_result));
  ASSERT_EQ(OB_SUCCESS, union_all.open());
  const ObRow *row = NULL;
  const ObObj *cell = NULL;
  uint64_t table_id = OB_INVALID_ID;
  uint64_t column_id = OB_INVALID_ID;
  int64_t int_value = -1;
  ASSERT_EQ(OB_SUCCESS, union_all.get_next_row(row));
  ...
  ASSERT_EQ(OB_ITER_END, union_all.get_next_row(row));
  ASSERT_EQ(OB_ITER_END, union_all.get_next_row(row));
  ASSERT_EQ(OB_ITER_END, union_all.get_next_row(row));

}
TEST(ObUnionAll, test3)
{
  const uint64_t left_table_id = 1001;
  const uint64_t right_table_id = 2001;
...
TEST(ObUnionDistinct, test3)
{
  ...
  ObMergeUnion union_distinct;
  union_distinct.set_distinct(true);
  ASSERT_EQ(OB_SUCCESS, union_distinct.set_child(0, first_query_result));
  ASSERT_EQ(OB_SUCCESS, union_distinct.set_child(1, second_query_result));
  ASSERT_EQ(OB_SUCCESS, union_distinct.open());
  const ObRow *row = NULL;
  const ObObj *cell = NULL;
  uint64_t table_id = OB_INVALID_ID;
  uint64_t column_id = OB_INVALID_ID;
  ...
  // 1
  ASSERT_EQ(OB_SUCCESS, union_distinct.get_next_row(row));
  ASSERT_EQ(OB_SUCCESS, row->raw_get_cell(0, cell, table_id, column_id));
  ASSERT_EQ(OB_SUCCESS, cell->get_int(int_value));
  ASSERT_EQ(1, int_value);
  ASSERT_EQ(left_table_id, table_id);
...
TEST(ObUnionDistinct, test1)
{
  ...
  TBSYS_LOG(INFO, "******************size=%d", multi_logic_plan->size());
}
