...
namespace oceanbase
{
  namespace common
  {
    ...
    int DES_crypt::stringTobyte(char* dest, const int64_t dest_len, const char* src, const int64_t src_len)
    {
      int err = OB_SUCCESS;
      if(dest == NULL || src == NULL || dest_len < src_len * 8)
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "invalid param. dest = %p, src = %p, dest_len = %ld, src_len = %ld", dest, src, dest_len, src_len);
      }
      int64_t tmp = 0;
      if(OB_SUCCESS == err)
      {
    ...
    int DES_crypt::byteTostring(char* dest, const int64_t dest_len, const char *src, const int64_t src_len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "invalid argument. dest=%p, dest_len=%ld, src=%p, src_len=%ld", dest, dest_len, src, src_len);
      }
      int32_t op = 0x0000;
      if(OB_SUCCESS == err)
      {
        for(int64_t i = 0; i < src_len; i++)
        {
          ...
          {
            err = OB_INVALID_ARGUMENT;
            TBSYS_LOG(WARN, "invalid argument, char is [%c], but should be 0 or 1", src[i]);
            break;
          }
          else
          {
    ...
    int DES_crypt::pad(char* dest, const int64_t dest_len, int64_t &slen)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "invalid argument. dest =%p, dest_len = %ld, slen = %ld, OB_MAX_TOKEN_BUFFER_LENGTH =%ld", dest, dest_len, slen, OB_MAX_TOKEN_BUFFER_LENGTH);
      }
      if(OB_SUCCESS == err)
      {
    ...
    int DES_crypt::unpad(char *src, int64_t &slen)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        TBSYS_LOG(WARN, "invalid argument, src =%p, slen = %ld, OB_MAX_TOKEN_BUFFER_LENGTH= %ld",src, slen, OB_MAX_TOKEN_BUFFER_LENGTH);
      }
      else
      {
        for(int i = original_txt_length_; i < OB_MAX_TOKEN_BUFFER_LENGTH; i++)
        {
          ...
          {
            err = OB_ERROR;
            TBSYS_LOG(WARN, "fail in unpad txt. the char is '%c', but should be '$'",src[i]);
          }
        }
      }
      if(OB_SUCCESS == err)
