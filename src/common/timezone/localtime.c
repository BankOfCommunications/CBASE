...
#endif /* defined ALL_STATE */
	if (tzload("/etc/localtime", lclptr, TRUE) != 0)
		gmtload(lclptr);
	settzname();
}
	...
	*/
	...
	for ( ; ; ) {
		...
		if (dir != 0) {
			...
			//mod peioy [DST_FIX] 20151126:b
			//if (lo > hi)
			//	return WRONG;
			if (lo > hi)
			{
				//check if yourtm is not exist DST time
				if (0 < mytm.tm_isdst 
					&& 0 >= yourtm.tm_isdst
					&& mytm.tm_year == yourtm.tm_year
					&& mytm.tm_mon == yourtm.tm_mon
					&& mytm.tm_mday == yourtm.tm_mday
					&& mytm.tm_hour == yourtm.tm_hour + 1)
				{
					/* skew diff deconds*/
					t += yourtm.tm_min * SECSPERMIN + yourtm.tm_sec - 1;
					break;
				}
				else
					return WRONG;
