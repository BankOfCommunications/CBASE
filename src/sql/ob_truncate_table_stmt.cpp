 ...
 * Version: $Id$
 *
 * ob_truncate_table_stmt.cpp
 *
 * Authors:
 *   Guibin Du <tianguan.dgb@taobao.com>
 *
 */
#include "ob_truncate_table_stmt.h"
#include "ob_schema_checker.h"

using namespace oceanbase::sql;
using namespace oceanbase::common;

ObTruncateTableStmt::ObTruncateTableStmt(ObStringBuf* name_pool)
  : ObBasicStmt(ObBasicStmt::T_TRUNCATE_TABLE)
{
...
int ObTruncateTableStmt::add_table_name_id(ResultPlan& result_plan, const ObString& table_name)
{
...
void ObTruncateTableStmt::print(FILE* fp, int32_t level, int32_t index)
{
  UNUSED(index);
  print_indentation(fp, level);
  fprintf(fp, "ObTruncateTableStmt %d Begin\n", index);
  if (if_exists_)
  {
  ...
  fprintf(fp, "\n");
  print_indentation(fp, level);
  fprintf(fp, "ObTruncateTableStmt %d End\n", index);
}
