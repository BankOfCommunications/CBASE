...
namespace oceanbase
{
  namespace sql
  {
    ...
    {
      int32_t ret = 0;
      char buf[common::OB_CHAR_SET_NAME_LENGTH];
      int n = snprintf(buf, common::OB_CHAR_SET_NAME_LENGTH, "%.*s", name.length(), name.ptr());
      if (n <= 0 || n >= common::OB_CHAR_SET_NAME_LENGTH)
      {
        ret = common::OB_BUF_NOT_ENOUGH;
        TBSYS_LOG(ERROR, "failed to dump charset[%.*s]", name.length(), name.ptr());
      }
      else
      {
        ret = get_char_number_from_name(buf);
      }
