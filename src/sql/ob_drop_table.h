  ...
  namespace sql
  {
    class ObDropTable: public ObNoChildrenPhyOperator
    {
      public:
        ...
        void set_sql_context(const ObSqlContext &context);//add liumz, [drop table -> clean table priv]20150902
        void set_if_exists(bool if_exists);
        int add_table_name(const common::ObString &tname);
        int add_table_id(const uint64_t &tid);//add liumz, [drop table -> clean table priv]20150902
      ...
      private:
        // types and constants
      private:
        void clean_owner_privilege();//add liumz, [drop table -> clean table priv]20150902
        // disallow copy
