/****************************************************************
 * Copyright (c) 2007, 2012, Oceanbase and/or its affiliates. All rights reserved.
 *
 * This file is part of MySQL5.5 release with some works to fit Oceanbase.
 *
 * Thanks for MySQL team and David M. Gay, the author of this software.
 */
...
size_t my_fcvt(double x, int precision, char *to, bool *error)
{
  ...
  DEBUG_ASSERT(precision >= 0 && precision < NOT_FIXED_DEC && to != NULL);
...
double my_strtod(const char *str, char **end, int *error)
{
  char buf[DTOA_BUFF_SIZE];
  double res;
  DEBUG_ASSERT(end != NULL && ((str != NULL && *end != NULL) ||
                              (str == NULL && *end == NULL)) &&
              error != NULL);
