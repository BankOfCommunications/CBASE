...
namespace oceanbase 
{
  namespace sstable 
  {
    ...
    ObAIOBuffer::ObAIOBuffer() : inited_(false), state_(FREE), fd_(-1), 
      sstable_id_(OB_INVALID_ID), fileinfo_cache_(NULL), file_info_(NULL), 
      file_offset_(-1), misalign_buf_(NULL), buffer_(NULL), buf_size_(0), 
      buf_read_(0), buf_to_read_(0), buf_pos_(0), block_(NULL), block_count_(0), 
      cur_block_idx_(0)
    {
    ...
    int ObAIOBuffer::init()
    {
      ...
      if (NULL == misalign_buf_)
      {
        misalign_buf_ = static_cast<char*>(ob_malloc(OB_DIRECT_IO_ALIGN + DEFAULT_AIO_BUFFER_SIZE,
        ...
        {
          buffer_ = reinterpret_cast<char*>(upper_align(reinterpret_cast<int64_t>(misalign_buf_), 
                                                        OB_DIRECT_IO_ALIGN));
                                   ...
                                   const bool reverse_scan)
    {
      ...
      else
      {
        ...
        buf_to_read_ = upper_align(block[0].offset_ + read_size, OB_DIRECT_IO_ALIGN) 
                       - file_offset_;
        if (buf_to_read_ > buf_size_)
        {
          TBSYS_LOG(WARN, "too many block to read, buf_to_read_=%ld, buf_size=%ld",
                    buf_to_read_, buf_size_);
          file_offset_ = -1;
          buf_to_read_ = 0;
          ret = OB_ERROR;
        }
    ...
    int ObAIOBuffer::get_block(const uint64_t sstable_id, const int64_t offset,
                                          ...
                                          const bool reverse_scan)
    {
      ...
      else
      {
        ...
        if (buf_to_read_ > buf_size_)
        {
          TBSYS_LOG(WARN, "too many blocks to read, buf_to_read_=%ld, buf_size=%ld",
                    buf_to_read_, buf_size_);
          buf_to_read_ = 0;
          ret = OB_ERROR;
        }
    ...
    int ObAIOBufferMgr::ensure_block_buf_space(const int64_t size)
    {
      ...
      {
        new_buf = static_cast<char*>(ob_malloc(buf_size, ObModIds::OB_SSTABLE_AIO));
        if (NULL == new_buf)
        {
                               ...
                               const bool reverse_scan)
    {
      ...
      {
        TBSYS_LOG(WARN, "two aio buffer aren't in FREE state, cur_aio_state=%s",
      ...
      if (OB_SUCCESS == ret && inited_)
      {
        ...
        ret = ensure_block_buf_space(block_infos.block_count_ * sizeof(ObBlockInfo));
      }

      if (OB_SUCCESS == ret && inited_)
      {
        next_offset = block_infos.position_info_[0].offset_;
        for (int64_t i = 0; i < block_infos.block_count_ && OB_SUCCESS == ret; ++i)
        {
          if (block_infos.position_info_[i].offset_ < 0 
              || block_infos.position_info_[i].size_ <= 0)
          {
            TBSYS_LOG(WARN, "invalid block info, block_offset=%ld, block_size=%ld,"
                            ", block_idx=%ld", 
          ...
          else if (!can_fit_aio_buffer(block_infos.position_info_[i].size_))
          {
            ...
            TBSYS_LOG(WARN, "block size is too big, block_size=%ld, block_idx=%ld,"
                            "max_block_size=%ld", 
                      block_infos.position_info_[i].size_, i, 
          ...
          else  //non-continuous blocks to read
          {
            reset();
            TBSYS_LOG(WARN, "blocks are not continuous, pre_offset=%ld, pre_size=%ld, "
                            "cur_offset=%ld, expected_offset=%ld, block_idx=%ld", 
                      i == 0 ? 0 : block_[i - 1].offset_, i == 0 ? 0 : block_[i - 1].size_,
                                               ...
                                               const bool check_cache)
    {
      ...
      {
        TBSYS_LOG(WARN, "invalid aio buffer index, aio_buf_idx=%ld", aio_buf_idx);
        ret = OB_ERROR;
      }
      else if (check_cache)
      {
                                   ...
                                   const char* buffer)
    {
      ...
      dataindex_key.sstable_id = sstable_id;
      dataindex_key.offset = offset;
      dataindex_key.size = nbyte;
    ...
    int ObAIOBufferMgr::get_block(ObBlockCache& block_cache,
                                  const uint64_t sstable_id,
                                  const int64_t offset,
                                  const int64_t size,
    ...
    void ObAIOBufferMgr::copy_remain_block_to_cache(ObAIOBuffer& aio_buf)
    {
      ...
      while (true)
      {
        status = aio_buf.get_next_block(dataindex_key.sstable_id,
                                        dataindex_key.offset,
                                        dataindex_key.size, 
                                        value.buffer, cached_);
                                                               ...
                                                               const bool free_mgr)
    {
      ...
      int64_t disk_no = get_ups_sstable_disk_no(sstable_id);
      //mod:e
      ...
      for (i = 0; i < item_count_; ++i)
      {
        if (sstable_id == item_array_[i].sstable_id_ 
            ...
            && (!free_mgr || (free_mgr && item_array_[i].aio_buf_mgr_->check_aio_buf_free())))
        {
          aio_buf_mgr = item_array_[i].aio_buf_mgr_;
          if (NULL == aio_buf_mgr)
          {
